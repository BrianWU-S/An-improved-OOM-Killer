...
/************************/
#include <linux/kernel.h>				//these are the extra needed head files for work function
...
#include <linux/oom.h>						// from here we know that is can use the function in oom_killer.c
...
/*
* From here we begin to define several work function and global variables
*1.  First two global variables indicates whether the module is loaded, only after the module is loaded, 
*we can start to use the new OOM Killer
*2.  Third and forth variables is the RSS collection table and mm_limit table for each user 
*3.  The following several funcitons help check the trigger condition of new OOM Killer
*
*/
...
/*
 * This is the 'heart' of the zoned buddy allocator. See line 2639 for better understanding
 */
struct page *
...
out:
	...
	/* 
	*Check new OOM killer condition here, 
	*These two flag variables' declaration is on line 2435, and it's defined on init_task.c
	*check_OOML_condition() is defined on line 2548
	*/
	//printk("THis is alloc_page, the MM_Limit_Module_Ready is %d, and the Check_RSS_Module_Ready is %d \n",MM_Limit_Module_Ready,Check_RSS_Module_Ready);
	if(Check_RSS_Module_Ready==383){
		// when two module are both installed, we can use new OOM killer here, otherwise, we just use the original OOM Killer
		long tmp_uid;
		int R_index;
		int L_index;
		tmp_uid=check_OOML_condition(&L_index,&R_index);
		if(tmp_uid==-1) goto MY_RETURN;				// not satisfy trigger condition
		else{
			// trigger new OOM killer
			printk("Start new my_out_of_memory!\n");
			my_out_of_memory(zonelist, gfp_mask, order, nodemask, false,tmp_uid,L_index,R_index);
		}		 
	}
